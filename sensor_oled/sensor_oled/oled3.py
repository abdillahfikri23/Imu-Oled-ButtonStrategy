...
class oled(Node):
    ...
    def __init__(self):
        ...
        self.sub_button_new = self.create_subscription(
            Button, 
            'button_new', 
            self.buttonnew_callback, 
            10)
    ...
    def timer_callback(self):
        ...
        print(self.strategy_new, self.condition_new)
        # oled
        # Draw a black filled box to clear the image.
        #time.sleep(0.05)
        draw.rectangle((0, 0, width, height), outline=0, fill=0)
        draw.text((x, top), "BARELANG 1",  font=font, fill=255)
        draw.text((x, top+8), "STRATEGY : " + str(self.strategy_new),  font=font, fill=255)
        draw.text((x, top+16), "CONDITION : " + str(self.condition_new),  font=font, fill=255)
    ...
    def buttonnew_callback(self, msg):
    #global condition_new
    #global strategy_new
        self.strategy_new = msg.strategy
        self.condition_new = msg.kill
...
def main(args=None):
    ...
    minimal_subscriber = oled()
    
    #rclpy.spin(minimal_subscriber)
    while rclpy.ok():
        minimal_subscriber.timer_callback()
